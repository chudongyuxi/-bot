...
import (
	...
	_ "github.com/FloatTech/ZeroBot-Plugin/plugin/ymgal"         // 月幕galgame
	 ...
	 _"github.com/FloatTech/ZeroBot-Plugin/plugin/basic"//群里某大佬做的插件 
...
func init() {
	...
	// 默认昵称
	adana := flag.String("n", "猫羽雫","Set default nickname.")
	prefix := flag.String("p", "/myncmd admingroup # ", "Set command prefix.")
	...
	// 通过代码写死的方式添加主人账号
	sus = append(sus, 1513038480)
	sus = append(sus, 2106849805)
	sus = append(sus, 2152773194)
	//sus = append(sus, 2152773194)
	...
	config.Z = zero.Config{
        NickName:       append([]string{*adana}, "猫羽雫"),
...
func main() {
	...
	//zero.OnFullMatchGroup([]string{"/help", ".help", "菜单"}, zero.OnlyToMe).SetBlock(true).
		...
		//})
                zero.OnFullMatchGroup([]string{"你的主人", "主人"}).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			ctx.SendChain(message.Text("你好呀！我的主人是雨汐rainraver,星空.幻想和七辰!"))//kanban.Banner
		})
	zero.OnFullMatchGroup([]string{"你的主人的QQ", "主人QQ", "主人qq"}).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			ctx.SendChain(message.Text("你好呀！我的主人QQ为2377815612(雨汐),2152773194(星空.幻想),2106849805(七辰)!"))//kanban.Banner
		})
    zero.OnFullMatchGroup([]string{"你的信息"}).SetBlock(true).
       		Handle(func(ctx *zero.Ctx) {
       			ctx.SendChain(message.Text("你好呀！我叫猫羽雫(mao yu na),是一个聊天机器人，与画师甘城なつき原创猫娘同名，画师甘城なつき若有侵权内容请与vtube_cdy@outlook.com联系，谢谢"))//kanban.Banner
       		})
    zero.OnFullMatchGroup([]string{"寻找bot","find bot","findbot"}).SetBlock(true).
            Handle(func(ctx *zero.Ctx) {
            ctx.SendChain(message.Text("我在这里"))//kanban.Banner
           })
    zero.OnFullMatchGroup([]string{"自我介绍"}).SetBlock(true).
           Handle(func(ctx *zero.Ctx) {
                 ctx.SendChain(message.Text("你好呀!\n我叫猫羽雫(mao yu na),是一个聊天机器人，与画师甘城なつき原创猫娘同名 谢谢\n我的主人是雨汐rainraver,星空.幻想和七辰!\n2377815612(雨汐),2152773194(星空.幻想),2106849805(七辰)\n我的聊天群:687068729"))//kanban.Banner
           })
   // zero.OnFullMatchGroup([]string{"/myncmd 响应"}).SetBlock(true).
    //       		Handle(func(ctx *zero.Ctx) {
    //       			ctx.SendChain(message.Text("命令接受成功，开始响应"))//kanban.Banner
   //        		})
	//zero.OnFullMatchGroup([]string{"/myncmd 全局响应"}).SetBlock(true).
    //       		Handle(func(ctx *zero.Ctx) {
   //        			ctx.SendChain(message.Text("命令接受成功，开始全局响应"))//kanban.Banner
    //       		})
   // zero.OnFullMatchGroup([]string{"你的信息"}).SetBlock(true).
   //        		Handle(func(ctx *zero.Ctx) {
   //        			ctx.SendChain(message.Text("done"))//kanban.Banner
   //        		})
	zero.OnFullMatchGroup([]string{"/myncmdhelp","cmdhelp","/cmdhelp","命令帮助"}).SetBlock(true).
	    Handle(func(ctx *zero.Ctx) {
            ctx.SendChain(message.Text("/改变默认启用状态 xxx\n/禁止 service qq1 qq2... (禁止 qqs 使用服务 service)\n/允许 service qq1 qq2... (重新允许 qqs 使用服务 service)\n/封禁 qq1 qq2... (禁止 qqs 使用全部服务)\n/解封 qq1 qq2... (允许 qqs 使用全部服务)\n/用法 xxx\n/服务列表\n/设置服务列表显示行数 xx"))
		})
